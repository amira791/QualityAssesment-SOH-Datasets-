...
if COL_MAP.get('discharge_capacity') and COL_MAP['discharge_capacity'] in cycle_data.columns:
    ...
    for cell_id, grp in cycle_data.groupby('file_source'):
        # Get first valid discharge capacity
        ...
        soh_records.append(grp[['file_source', 'chemistry', 'temperature_C', 
    ...
    if soh_records:
        ...
        # Plot 1: Capacity fade by chemistry
        ax1 = axes[0, 0]
        ...
        # Plot 4: Cycle life distribution
        ax4 = axes[1, 1]
        cycle_life = soh_stats.groupby('chemistry')['total_cycles'].agg(['mean', 'std', 'count'])
        if len(cycle_life) > 0:
            x_pos = np.arange(len(cycle_life))
            ax4.bar(x_pos, cycle_life['mean'], yerr=cycle_life['std'], 
                    capsize=5, alpha=0.7, color=['#1f77b4', '#ff7f0e', '#2ca02c'])
            ax4.set_xticks(x_pos)
            ax4.set_xticklabels(cycle_life.index)
            ax4.set_ylabel('Cycle Life (cycles to EOL or end of test)')
        ...
        for i, cell in enumerate(selected_cells):
            ...
            if len(cell_data) > 0:
                ...
                axes_flat[i].axhline(0.8 * cell_data[cap_col].iloc[0], 
                                    color='r', linestyle='--', linewidth=0.8)
                axes_flat[i].set_title(f'{os.path.basename(cell)}\n{chem}, {temp}°C')
...
Batteries    : {cycle_data['file_source'].nunique() if 'file_source' in cycle_data.columns else 'N/A'}
Chemistries  : {', '.join([c for c in cycle_data['chemistry'].unique() if c != 'unknown']) if 'chemistry' in cycle_data.columns else 'N/A'}

Test Conditions:
  Temperatures  : {sorted([t for t in cycle_data['temperature_C'].unique() if t > -100]) if 'temperature_C' in cycle_data.columns else 'N/A'} °C
  DoD Ranges    : {cycle_data['DoD_range'].unique().tolist() if 'DoD_range' in cycle_data.columns else 'N/A'}
  Charge C-rates: {sorted(cycle_data['charge_Crate'].unique()) if 'charge_Crate' in cycle_data.columns else 'N/A'}C
  Discharge C-rates: {sorted(cycle_data['discharge_Crate'].unique()) if 'discharge_Crate' in cycle_data.columns else 'N/A'}C

Efficiency:
  Coulombic efficiency  : {cycle_data[COL_MAP['coulombic_efficiency']].mean():.2f} ± {cycle_data[COL_MAP['coulombic_efficiency']].std():.2f}% if 'coulombic_efficiency' in COL_MAP and COL_MAP['coulombic_efficiency'] in cycle_data.columns else 'N/A'}
  Energy efficiency     : {'Available' if 'energy_efficiency' in COL_MAP and COL_MAP['energy_efficiency'] in cycle_data.columns else 'Not available'}

Temperature:
  Operating range       : {cycle_data[COL_MAP['min_temp']].min():.1f} – {cycle_data[COL_MAP['max_temp']].max():.1f} °C if 'min_temp' in COL_MAP and 'max_temp' in COL_MAP and COL_MAP['min_temp'] in cycle_data.columns and COL_MAP['max_temp'] in cycle_data.columns else 'N/A'}
""")
